...
extern int width,height; /* the board size 棋盘大小*/

extern int info_timeout_turn;  /* time for one turn in milliseconds每一轮的时间 */
extern int info_timeout_match; /* total time for a game 游戏总时间*/
extern int info_time_left;  /* remaining time for a game 游戏剩余时间*/
extern int info_max_memory; /* maximum memory in bytes, zero if unlimited最大存储 */
extern int info_game_type;  /* 0:human opponent, 1:AI opponent, 2:tournament, 3:network tournament 游戏种类*/
extern int info_exact5;     /* 0:five or more stones win, 1:exactly five stones win 判断几颗棋会赢 */
extern int info_continuous; /* 0:single game, 1:continuous 判断是单一还是连续游戏*/
extern int terminateNew; /* return from brain_turn when terminate>0 */
extern unsigned start_time; /* tick count at the beginning of turn 记录开始时间*/
extern char dataFolder[256]; /* folder for persistent files持续的文件的文件夹 */
